...
void tmr_setup_period(int timer, int ms)
{
    ...
    /*if (ms < 6)
    {
        prescaler = 8.0;
        if(timer == TIMER1){
         // set the prescaler 0:0, 1:8, 2:64, 3:256
            T1CONbits.TCKPS = 1;
            ...
            T2CONbits.TCKPS = 1; // set the prescaler 0:0, 1:8, 2:64, 3:256        T1CONbits.TCKPS = 1;
    ...
    else if (ms < 56 && ms >= 6)
    {
        ...
        if(timer == TIMER1){
         // set the prescaler 0:0, 1:8, 2:64, 3:256
            ...
            T2CONbits.TCKPS = 2; // set the prescaler 0:0, 1:8, 2:64, 3:256        T1CONbits.TCKPS = 1;
            TMR2 = 0; // reset timer count
            IFS0bits.T2IF = 0; // set the flag to zero  
            PR2 = ((((float) FCY) / prescaler) * (((float) ms) / toSec));
            T2CONbits.TON = 1; // start timer
...
{
    // return 1 if timer has esxpired, otherwise returns 0
    if(timer == TIMER1){
        if (IFS0bits.T1IF == 1)
        {
            IFS0bits.T1IF = 0;
